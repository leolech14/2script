...
class Transaction:
    ...
    def _generate_hash(self) -> str:
        ...
        hash_input = (
            f"{self.card_last4}|{self.post_date}|{self.desc_raw}|"
            f"{self.amount_brl}|{self.installment_tot}|{self.category}"
        )
        return hashlib.sha1(
            ...
            with pdfplumber.open(str(pdf_path)) as pdf:
                ...
                for page_num, page in enumerate(pdf.pages, 1):
                    ...
                    if text:
                        ...
                        logger.debug(
                            f"Page {page_num}: extracted {len(page_lines)} lines"
                        )
...
class CardNumberExtractor:
    ...
    def extract_card_numbers(self, lines: list[str]) -> dict[str, str]:
        ...
        logger.info(
            f"Extracted {len(unique_cards)} unique card numbers: {unique_cards}"
        )
...
class TransactionExtractor:
    ...
    def _compile_patterns(self):
        ...
        self.patterns = {
            "payment": re.compile(
                r"^(?P<date>\d{1,2}/\d{1,2}(?:/\d{4})?)\s+PAGAMENTO.*?(?P<amt>-?\s*[\d.,]+)\s*$",
                re.I,
            ),
            "domestic": re.compile(
                r"^(?P<date>\d{1,2}/\d{1,2})\s+(?P<desc>.+?)\s+(?P<amt>[-\d.,]+)$"
            ),
            "fx_main": re.compile(
                r"^(?P<date>\d{2}/\d{2})\s+(?P<desc>.+?)\s+(?P<orig>[\d.,]+)\s+(?P<brl>[\d.,]+)$"
            ),
            "fx_city": re.compile(
    ...
    def _parse_fx_transaction(
        self, lines: list[str], card_last4: str
    ) -> Transaction | None:
    ...
    def _parse_payment(
        self, line: str, card_last4: str, payment_count: int
    ) -> Transaction | None:
    ...
    def _parse_domestic_transaction(
        self, line: str, card_last4: str
    ) -> Transaction | None:
...
class ItauParser:
    ...
    def validate_against_golden(
        self, transactions: list[Transaction], golden_path: Path
    ) -> dict:
...
def main():
    ...
    parser.add_argument(
        "-c", "--config", type=str, default=CONFIG_FILE, help="Configuration file"
    )
    parser.add_argument(
    ...
    try:
        itau_parser = ItauParser(args.config)
        transactions = itau_parser.parse_pdf(args.pdf_file, output_path)
