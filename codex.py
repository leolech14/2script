...
def main():
    ...
    for f in a.files:
        ...
        pdf_metrics = {
            ...
            "Lançamentos atuais": sum(float(r["valor_brl"]) for r in rows_dedup if r["categoria_high"] not in ("PAGAMENTO", "AJUSTE")),
            "Total desta fatura": sum(float(r["valor_brl"]) for r in rows_dedup if r.get("valor_brl") not in ("", None)),
            "Nº de pagamentos 7117": len([r for r in rows_dedup if r["categoria_high"] == "PAGAMENTO"]),
            "Valor total dos pagamentos": sum(float(r["valor_brl"]) for r in rows_dedup if r["categoria_high"] == "PAGAMENTO"),
            "Valor do maior pagamento": min((float(r["valor_brl"]) for r in rows_dedup if r["categoria_high"] == "PAGAMENTO"), default=0),
            ...
            "Valor total lançamentos internacionais (BRL)": valor_fx + sum(float(r.get("iof_brl", 0) or 0) for r in rows_dedup if r["categoria_high"] == "IOF"),
