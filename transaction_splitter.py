...
def extract_card_from_description(desc: str) -> str | None:
...
def process_csv(input_file: Path, output_file: Path):
    ...
    # Show sample splits
    print("\nSample splits:")
    for i, (orig, split) in enumerate(zip(rows[:3], [splits[:2] for splits in [split_transaction_line(r['description'], r['amount'], r['date']) for r in rows[:3]]], strict=False)):
